...
//result & display
	...
	line(draw, Point(_pt1.x, _pt1.y), Point(_pt2.x, _pt2.y), Scalar(0, 0, 255), thickness, lineType);
...
void COpenCVAppGUIDlg::OnReleaseBuffer()
{
	_mMatBuff.clear();

	/*for (auto iter = _mMatBuff.begin(); iter != _mMatBuff.end(); iter++) {
		_mMatBuff.erase(iter->first);
	...
	*/
	...
	for (size_t row = 0; row < rows; row++)
	{
		for (size_t col = 0; col < cols; col++)
		{
			...
			if ((pDst[index]-pDst[index-1] == 255)||(pDst[index-1]-pDst[index]==255))
			{
				if (row == 0)
				{
					_pt1.x = col;
